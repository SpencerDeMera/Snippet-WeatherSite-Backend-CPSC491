...
router.get("/find_photo", cache("30 minutes"), async (req, res) => {
  try {
    ...
    const resp_ref = await axios.get(
      `${BASE_URL}/findplacefromtext/json?${params_ref}`
    );
    ...
    const result = [
      "https://",
      resp_photo.request.socket._host,
      resp_photo.request.path,
    ].join("");
